...
class vgg16(Network):
  ...
  def get_variables_to_restore(self, variables, var_keep_dic):
    ...
    for v in variables:
      ...
      if v.name == 'vgg_16/conv1T/conv1T_1/weights:0':
        self._variables_to_fix[v.name.replace('T','')] = v
        continue
      ...
      #  v1 = v.name.split(':')[0]
      #  print('Variables restored: %s' % v.name)
      #  variables_to_restore[v1]=v
      #  continue
      v1 = v.name.split(':')[0]
      if v1.replace('T','') in var_keep_dic:
        print('Variables restored: %s' % v.name)
        variables_to_restore[v1.replace('T','')]=v
      #if v1.replace('RGB','') in var_keep_dic:
      #  print('Variables restored: %s' % v.name)
      #  variables_to_restore[v1.replace('RGB','')]=v
